...
async function buildDb() {
  if (isNeon) {
    ...
    return { db: drizzle(neon(url)), excipientesRotulo, operadores, tintas, end: async () => {} };
  }
  const { default: postgres } = await import('postgres');
  const { drizzle } = await import('drizzle-orm/postgres-js');
  const { excipientesRotulo, operadores, tintas } = await import('../src/db/schema');
  const client = postgres(url);
  return { db: drizzle(client), excipientesRotulo, operadores, tintas, end: async () => client.end() };
}

async function main() {
  const { db, excipientesRotulo, operadores, tintas, end } = await buildDb();
